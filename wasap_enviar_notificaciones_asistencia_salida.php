...
<h6>ENVIO MASIVO DE NOTIFICACIONES A PADRES DE FAMILIA</h6>
...
<button class="btn btn-danger btn-sm" type="submit">Enviar
                                                Mensajes</button>
</div>
</div>
</div>
...
$(document).ready(function() {
    $('#formEnviarMensaje').on('submit', function(e) {
        ...
        $.ajax({
            url: 'wasap_obtener_asistencias_salidas.php',
            type: 'POST',
            data: {
                fecha: fecha
            },
            success: function(response) {
                ...
                function enviarMensaje(i) {
                    if (i < estudiantes.length) {
                        // 🟢 Crear el mensaje personalizado aquí
                        var entradaHora = new Date(estudiantes[i].salida).toLocaleTimeString('es-PE', {
                            hour: '2-digit',
                            minute: '2-digit'
                        });

                        var mensaje = `Sr. padre/madre, su hijo(a) ${estudiantes[i].names} ${estudiantes[i].surnames} acaba de salir a las ${entradaHora}.`;

                        $.ajax({
                            url: 'wasap_enviar_notificaciones_entrada.php',
                            type: 'POST',
                            data: {
                            ...
                            success: function() {
                                console.log("Mensaje enviado a " + estudiantes[i].telefono);
                                setTimeout(function() {
                                    enviarMensaje(i + 1);
                                }, 5000);
                            }
                        ...
                        Swal.fire({
                            ...
                            text: 'Todos los mensajes fueron enviados.',
