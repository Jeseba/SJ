...
<h6>ENVIO MASIVO DE MENSAJES</h6>
<hr/>
</div>
...
<label for="T_Mensaje">Mensaje :</label>
<div class="position-relative">
<textarea class="form-control" id="T_Mensaje" name="T_Mensaje" placeholder="Ingrese su Mensaje" required="" rows="4"></textarea>
</div>
</div>
<div class="modal-body">
<div class="mb-3">
<div class="form-check form-switch mt-4">
<input class="form-check-input" id="Che_General" name="Che_General" type="checkbox"/>
<label class="form-check-label" for="Che_General">Envio
                                                                General</label>
</div>
<div class="row">
...
<button class="btn btn-success btn-sm" type="submit">Enviar
                                                Mensajes</button>
</div>
</div>
</div></form>
...
document.addEventListener('DOMContentLoaded', function() {
    const checkbox = document.getElementById('Che_General');
    const comboGrados = document.getElementById('Combo_Grados');
    const comboSecciones = document.getElementById('Combo_Secciones');

    checkbox.addEventListener('change', function() {
        if (checkbox.checked) {
            comboGrados.disabled = true;
            comboSecciones.disabled = true;
        } else {
            comboGrados.disabled = false;
            comboSecciones.disabled = false;
        }
    });
});
</script>
...
$(document).ready(function() {
    $('#formEnviarMensaje').on('submit', function(e) {
        ...
        seccion = $('#Combo_Secciones').val();
        grado = $('#Combo_Grados').val();
        che_general = $('#Che_General').is(':checked') ? 1 : 0;
        // Mostrar alerta de "enviando..." con SweetAlert2
        Swal.fire({
            title: 'Enviando mensajes...',
            html: 'Por favor, espera mientras se envían los mensajes.',
        ...
        // Obtener la lista de estudiantes por AJAX
        $.ajax({
            url: 'wasap_obtener_estudiantes.php',
            type: 'POST',
            data: {
                seccion: seccion,
                grado: grado,
                che_general: che_general
            },
            success: function(response) {
                var estudiantes = JSON.parse(response);
                console.log('Json devuelto : ', estudiantes);

                function enviarMensaje(i) {
                    if (i < estudiantes.length) {
                        $.ajax({
                            url: 'wasap_enviar_sms.php',
                            type: 'POST',
                            data: {
                            ...
                            success: function() {
                                console.log("Mensaje enviado a " + estudiantes[
                                    i].telefono);
                                setTimeout(function() {
                                    enviarMensaje(i + 1);
                                }, 2000);
                        ...
                        // Cuando termine de enviar todos
            ...
            error: function() {
                Swal.close(); // Cerrar el "enviando..." si ocurre error
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
